defmodule MeetupWeb.Game do
  ...
  def mount(_params, _session, socket) do
    {:ok, assign(socket, gleam_store: :gleam_game.create_store(0))}
  end

  def handle_event(action, _data, socket) do
    IO.inspect({:elixir_before, socket.assigns.gleam_store})
    gleam_store = :gleam_game.update(socket.assigns.gleam_store, String.to_atom(action))
    IO.inspect({:elixir_after, gleam_store})
